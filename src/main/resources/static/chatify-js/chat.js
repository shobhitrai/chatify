function createMainChat(payload) {
   if (payload.status === 100) {
      ...
      data = '<a id="chatgroup-' + cg.senderId + '" ' +
         'href="#chat-' + cg.senderId + '" class="filterDiscussions all unread single" ' +
         'data-toggle="list"><img class="avatar-md" src="' + cg.senderProfileImage + '" ' +
         'data-toggle="tooltip" data-placement="top" title="' + cg.senderFirstName + '" ' +
         'alt="avatar"><div class="status"><i class="material-icons offline">' +
         'fiber_manual_record</i></div><div class="new bg-gray"><span>?</span></div>' +
         '<div class="data"><h5>' + cg.senderFirstName + ' ' + cg.senderLastName + '</h5>' +
