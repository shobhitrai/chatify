function createMainChat(payload) {
   if (payload.status === 100) {
      ...
      let context = '<a id="chatgroup-' + sender.userId + '" ' +
         'href="#" class="filterDiscussions all unread single" ' +
         'data-toggle="list"><img class="avatar-md" src="' + sender.profileImage + '" ' +
         'data-toggle="tooltip" data-placement="top" title="' + sender.firstName + '" ' +
         'alt="avatar"><div class="status"><i class="material-icons '+onlineStatus+'">' +
         'fiber_manual_record</i></div><div class="new bg-gray"><span>?</span></div>' +
         '<div class="data"><h5>' + sender.firstName + ' ' + sender.lastName + '</h5>' +
         '<span>' + chat.formattedDate + '</span><p id="p-' + sender.userId + '">' +
         chat.message + '</p></div></a>';
...
function appendFriendRequestChat(data) {
   ...
   if (sessionUserId === chat.senderId) {
      context = '<div class="babble tab-pane fade active show" id="chat-' + other.userId + '">' +
         '<div class="chat"> <div class="top"> <div class="container"> <div class="col-md-12">' +
         ' <div class="inside"> <a href="#"><img class="avatar-md" src="' + other.profileImage + '" ' +
         'data-toggle="tooltip" data-placement="top" title="' + other.firstName + '" alt="avatar"></a>' +
         ' <div class="status"> <i class="material-icons '+onlineStatus+'">fiber_manual_record</i>' +
         ' </div> <div class="data"> <h5><a href="#">' + other.firstName + ' ' + other.lastName + '</a></h5>' +
         ...
         'src="' + other.profileImage + '" data-toggle="tooltip" data-placement="top" ' +
         'title="' + other.firstName + '" alt="avatar"></a> <h5><span>"You have send a friend request to ' +
          other.firstName + ' ' +other.lastName+ ', Waiting for acceptance."</span></h5>' +
         ' <div class="options"><button class="btn button fr-cancel-btn" ' +
         'id="cancel-' + other.userId + '"><i class="material-icons">close</i></button> </div>' +
      ...
      context = '<div class="babble tab-pane fade active show" id="chat-' + other.userId + '">' +
         ...
         ' <div class="no-messages request"> <a href="#"><img class="avatar-xl" ' +
         'src="' + other.profileImage + '" data-toggle="tooltip" data-placement="top" ' +
         'title="' + other.firstName + '" alt="avatar"></a> <h5><span>' + chat.message + '</span></h5>' +
         ' <div class="options"> <button class="btn button fr-accept-btn" id="accept-' + other.userId + '">' +
         '<i class="material-icons">check</i></button> <button class="btn button fr-reject-btn" ' +
         'id="reject-' + other.userId + '"><i class="material-icons">close</i></button> </div>' +
         ' </div> </div> </div> </div> <div class="container"> <div class="col-md-12">' +
         ' <div class="bottom"> <form class="position-relative w-100"> <textarea class="form-control"' +
         ' placeholder="Messaging unavailable" rows="1" disabled></textarea> <button class="btn' +
         ' emoticons disabled" disabled><i class="material-icons">insert_emoticon</i></button>' +
...
function appendNoChatFoundScreen(data) {
    ...
    let context = '<div class="babble tab-pane fade active show" id="chat-' + other.userId + '">' +
               '<div class="chat">' +
                '<div class="top">' +
                    '<div class="container">' +
                        '<div class="col-md-12">' +
                            '<div class="inside">' +
                                '<a href="#"><img class="avatar-md" src="'+other.profileImage+
                                '" data-toggle="tooltip" data-placement="top" title="Lean" alt="avatar"></a>' +
                                '<div class="status">' +
                                    '<i class="material-icons '+status+'">fiber_manual_record</i>' +
                                '</div>' +
                                '<div class="data">' +
                                    '<h5><a href="#">'+other.firstName+' '+other.lastName+'</a></h5>' +
                                    '<span>'+status2+'</span>' +
                                '</div>' +
